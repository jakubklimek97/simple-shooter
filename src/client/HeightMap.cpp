...
bool CMultiLayeredHeightmap::LoadHeightMapFromImage(string sImagePath,const char *path,bool gamma, std::string &directory)
{
	if (bLoaded)
	{
		bLoaded = false;
...
{
	bool bOK = true;
	bOK &= shShaders[0].LoadShader("terrain.vert", GL_VERTEX_SHADER);
	bOK &= shShaders[1].LoadShader("terrain.frag", GL_FRAGMENT_SHADER);
